import data.{
  type Education, type JobExperience, type Language, type Skill, type Social,
  job_experience_list, list_of_education, list_of_languages, list_of_skills,
  list_of_socials,
}
import gleam/int
import gleam/list
import gleam/pair
import lustre
import lustre/attribute.{class, href, src, style, target, title}
import lustre/element.{type Element, text}
import lustre/element/html.{a, div, h1, h2, img, li, p, ul}
  ...
  li(
    ...
    [
      ...
      div([class("ml-4 grow-1 sm:grow-0 sm:min-w-60 overflow-hidden")], [
        p([class("text-xl font-medium text-black truncate")], [
          text(job_experience.company),
        ]),
        p([class("text-md text-slate-800 truncate")], [
          text(job_experience.position),
        ]),
        p([class("text-md text-stone-500 truncate")], [
          text(timestamp_difference(
            job_experience.start_date,
            job_experience.end_date,
          )),
        ]),
        p([class("text-md text-stone-500 truncate")], [
          text(job_experience.place_of_work),
        ]),
      ]),
      div(
        [class("shrink-0 grow-1 hidden sm:flex flex-row space-x-2")],
        job_experience.technologies
          |> list.map(fn(technology) {
            div(
              [
      ...
      div([class("relative h-[100px] w-10 shrink-0")], [
        p(
          ...
          [text(int.to_string(job_experience.start_date |> pair.first))],
        ),
        p(
          [
            class(
...
fn view_education(education: Education, is_last: Bool) -> Element(msg) {
  let append = case is_last {
    ...
    True -> [
      p(
        ...
        [text(int.to_string(education.end_year))],
      ),
    ]
  }
  ...
  li([class("py-3 flex flex-row items-center space-x-4")], [
    ...
    div([class("ml-4 shrink-1 grow-1 overflow-hidden")], [
      ...
      p([class("text-md text-slate-800 truncate"), title(education.field)], [
        text(education.field),
      ]),
      p([class("text-md text-stone-500 truncate"), title(education.degree)], [
    ...
    div([class("relative h-[100px] w-10 shrink-0")], [
      p(
        ...
        [text(int.to_string(education.start_year))],
      ),
      p(
        [
          class(
...
fn view(_model) {
  div([], [
    div([class("bg-white border-t-4 border-stone-100")], [
      div(
        [class("container max-w-150 mx-auto flex flex-col gap-4 items-center")],
        [
          img([
    ...
    div([class("bg-stone-50 px-10 py-10")], [
      div([class("container max-w-170 mx-auto flex flex-col gap-4")], [
        ...
        ul([], list.map(job_experience_list, view_job_experience)),
      ]),
    ]),
    ...
    div([class("bg-stone-100 px-10 py-10")], [
      div([class("container max-w-170 mx-auto flex flex-col gap-4")], [
        ...
        ul(
          [],
          list_of_skills
            |> list.filter(fn(skill: Skill) { skill.major == True })
            |> list.map(fn(skill: Skill) {
              li(
                ...
                [
                  div(
                    [class("flex flex-row items-center gap-2 min-w-[160px]")],
                    [
                      img([
                        class("w-6 h-6"),
                        src("./priv/static/lang/" <> skill.logo <> ".png"),
                      ]),
        ...
        ul(
          [],
          list_of_skills
            |> list.filter(fn(skill: Skill) { skill.major == False })
            |> list.map(fn(skill: Skill) {
              li(
                [
                  class(
            ...
            li(
              ...
              [
                div([class("flex flex-row items-center gap-2")], [
                  img([
                    class("w-6 h-6"),
                    src("./priv/static/social/" <> social.logo <> ".png"),
                  ]),
                  p([class("text-slate-100 text-xl min-w-[90px]")], [
                    text(social.name),
                  ]),
                ]),
                a(
